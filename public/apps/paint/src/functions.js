...
import { $G, E, JP_OS_INTERNAL_MARKER, TAU, debounce, from_canvas_coords, get_file_extension, get_help_folder_icon, get_icon_for_tool, get_rgba_from_color, is_discord_embed, is_pride_month, make_canvas, render_access_key, to_canvas_coords } from "./helpers.js";
...
async function file_open() {
	// JP-OS: Local file uploads are disabled to prevent security risks and align with JP-OS's restrictions.
	show_error_message("JP-OS: Local file uploads are disabled.");
	return;
}
...
function file_load_from_url() {
	// JP-OS: Loading from external URLs is disabled to prevent security risks and align with JP-OS's restrictions.
	show_error_message("JP-OS: Loading from external URLs is disabled.");
	return;
...
async function choose_file_to_paste() {
	// JP-OS: Local file uploads are disabled to prevent security risks and align with JP-OS's restrictions.
	show_error_message("JP-OS: Choosing local files to paste is disabled.");
	return;
...
function edit_copy(execCommandFallback) {
	...
	if (text.length > 0) {
		...
		selection.canvas.toBlob((blob) => {
			sanity_check_blob(blob, () => {
				// JP-OS: Include the internal marker along with the image.
				// This allows us to verify that pasted content originated from within the application.
				const itemData = {
					[blob.type]: blob,
					"text/plain": JP_OS_INTERNAL_MARKER,
				};
				navigator.clipboard.write([
					new ClipboardItem(itemData),
				]).then(() => {
					window.console?.log("Copied selection to the clipboard with JP-OS marker.");
				}, (error) => {
	...
	try {
		...
		for (const item of clipboardItems) {
			if (item.types.includes("text/plain")) {
				try {
					...
					if (text === JP_OS_INTERNAL_MARKER) {
						isInternal = true;
						break;
					}
				} catch (error) {
					window.console?.error("Failed to read text marker from clipboard item", error);
				}
			}
		}

		if (!isInternal) {
			show_error_message("Security: External images cannot be pasted into JPPaint. Only images copied from within JPPaint are allowed.");
			return;
		}
		...
		if (error.name === "NotFoundError") {
			try {
				const clipboardText = await navigator.clipboard.readText();
				if (clipboardText) {
					// JP-OS: If it's plain text, we check if it is the marker.
					// If it's just the marker string itself as a text paste, we probably don't want to paste it as a selection.
					if (clipboardText === JP_OS_INTERNAL_MARKER) {
						return;
					}

					const uris = get_uris(clipboardText);
					if (uris.length > 0) {
						// JP-OS: External image URLs are also forbidden for pasting.
						show_error_message("Security: External image URLs cannot be pasted as drawings.");
						/*
