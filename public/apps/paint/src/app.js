...
// Make the status text permanent
const status_text_permanent_html = $status_text.html();
$status_text.text = (text) => {
	if (text === undefined) return $status_text.get(0).textContent;
	return $status_text;
};
$status_text.html = (html) => {
	if (html === undefined) return status_text_permanent_html;
	return $status_text;
};
...
$("body").on("dragover dragenter", (/** @type {JQuery.DragOverEvent | JQuery.DragEnterEvent} */event) => {
	...
	if (has_files) {
		event.preventDefault();
		// JP-OS: Drag-and-drop file uploads are disabled to prevent security risks and align with JP-OS's restrictions.
		show_error_message("JP-OS: Drag-and-drop file uploads are disabled.");
		return;
...
$G.on("keydown", (e) => {
	console.log("JPPaint received key:", e.key, e.code); // JP-OS: Added for diagnostics
...
$G.on("cut copy paste", (e) => {
	...
	if (e.type === "copy" || e.type === "cut") {
		...
		// JP-OS: Use the secure edit_paste function that checks for the internal marker.
		// This replaces the manual item looping that allowed pasting external images/URLs.
		edit_paste();
	}
});
// #endregion
...
// JP-OS: Pre-fill canvas white synchronously so the session system's empty-canvas
...
localStore.get({
	...
	if (err) { return; }
	// Force canvas size to 256x256
	my_canvas_width = 256;
	my_canvas_height = 256;
